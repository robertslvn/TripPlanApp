...
(function () {
  ...
  _travelMap.createMap = function (options) {
    ...
    if (currentLocation != null) {
        ...
        map = new GMaps({
            div: settings.selector,
            lat: settings.stops[0].lat,
            lng: settings.stops[0].long,
            zoom: settings.initialZoom
        });
    }
    ...
    if (currentLocation != null) {
        currentLocation = settings.stops[settings.stops.length - 1];
        if (settings.stops[settings.stops.length - 1].info == settings.stops[0].info) {
            map.drawOverlay({
                lat: currentLocation.lat,
                lng: currentLocation.long,
                content: '<div class="' + settings.overlay.className + '">Start and End Point:<br/>' + settings.stops[settings.stops.length - 1].info + '<div class="' + settings.overlay.className + '-arrow ' + settings.overlay.className + '-arrow-above"></div></div>',
                verticalAlign: 'top',
        ...
        map.drawOverlay({
            lat: currentLocation.lat,
            lng: currentLocation.long,
            content: '<div class="' + settings.overlay.className + '">Final Destination:<br/>' + settings.stops[settings.stops.length-1].info + '<div class="' + settings.overlay.className + '-arrow ' + settings.overlay.className + '-arrow-above"></div></div>',
            verticalAlign: 'top',
            horizontalAlign: 'center',
            layer: "overlayImage"
        });
        map.drawOverlay({
            lat: settings.stops[0].lat,
            lng: settings.stops[0].long,
            content: '<div class="' + settings.overlay.className + '">Starting Point:<br/>' + settings.stops[0].info + '<div class="' + settings.overlay.className + '-arrow ' + settings.overlay.className + '-arrow-above"></div></div>',
            verticalAlign: 'top',
            horizontalAlign: 'center',
            layer: "overlayImage"
        });
        ...
        currentLocation = settings.stops[0];
        map.drawOverlay({
            lat: settings.stops[0].lat,
            lng: settings.stops[0].long,
            content: '<div class="' + settings.overlay.className + '">Starting Point:<br/>' + settings.stops[0].info + '<div class="' + settings.overlay.className + '-arrow ' + settings.overlay.className + '-arrow-above"></div></div>',
